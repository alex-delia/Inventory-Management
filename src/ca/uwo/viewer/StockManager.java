...
public class StockManager extends Viewer implements Runnable {
	...
	private StockManager() {
		...
		// restockDetails.put("onions", 50);
				...
				t.start();
				this.restockStrategy=RestockStrategyFactory.create("default");
	}
	...
	public void inform(Item item) {
		...
		//calculate the restock quantity using the strategy
		int restockQ = restockStrategy.calculateQuantity(item.getName(), item.getAvailableQuantity(), item.getPrice());
		
		restockDetails.put(item.getName(), restockQ);
	}
